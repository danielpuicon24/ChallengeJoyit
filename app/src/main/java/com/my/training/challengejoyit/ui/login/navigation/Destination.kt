package com.my.training.challengejoyit.ui.login.navigation

sealed class Destination(protected val route: String, vararg params: String) {
    val fullRoute: String = if (params.isEmpty()) route else {
        val builder = StringBuilder(route)
        params.forEach { builder.append("/{${it}}") }
        builder.toString()
    }

    sealed class NoArgumentsDestination(route: String) : Destination(route) {
        operator fun invoke(): String = route
    }

    object LoginScreen : NoArgumentsDestination(LOGIN_ROUTE)

    object SigninScreen : NoArgumentsDestination(SIGNIN_ROUTE)

    object SignupScreen : NoArgumentsDestination(SIGNUP_ROUTE)

    object DetailsScreen : NoArgumentsDestination(DETAIL_ROUTE)



    companion object {
        private const val LOGIN_ROUTE = "login"
        private const val SIGNIN_ROUTE = "signin"
        private const val SIGNUP_ROUTE = "signup"
        private const val DETAIL_ROUTE = "details"

    }
}

internal fun String.appendParams(vararg params: Pair<String, Any?>): String {
    val builder = StringBuilder(this)

    params.forEach {
        it.second?.toString()?.let { arg ->
            builder.append("/$arg")
        }
    }

    return builder.toString()
}